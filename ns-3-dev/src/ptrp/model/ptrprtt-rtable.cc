...
NS_LOG_COMPONENT_DEFINE ("ptrpRoutingTable");
...
void
RoutingTableEntry::SetRTTstartRrep(int64_t startStampRrep){
  rttStartStampRrep = startStampRrep;
}

int64_t
RoutingTableEntry::GetRTTstartRrep() const {
...
RoutingTable::Update (RoutingTableEntry & rt)
{
  ...
  stringstream ss;
  ss << rt.GetInterface().GetLocal();
  const char *nodeName = ss.str().c_str();
  
  stream.open(HOME + path + nodeName + ".txt", fstream::trunc);
  if(stream){
  ...
  for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i =
    ...
    {
      if(i->second.GetNextHop() != "127.0.0.1" &&i->second.GetNextHop() != "102.102.102.102" && i->second.GetNextHop() != "10.1.1.255"){

      
        stream << endl << i->second.GetNextHop();
        //stream << "\n";
