 ...
 */
#include "ptrp-packet.h"
#include "ns3/address-utils.h"
#include "ns3/packet.h"
#include "ptrp-neighbor.h"
#include <set>
#include <array>
#include <list>
#include <iostream>
...
{
  static TypeId tid = TypeId ("ns3::ptrp::TypeHeader")
    .SetParent<Header> ()
    .SetGroupName ("ptrp")
...
static int64_t rttStartStamp;

void
RreqHeader::SetRTTstart(int64_t startStamp){
  rttStartStamp = startStamp;
}

int64_t
RreqHeader::GetRTTstart() const {
  return rttStartStamp;
}
...
Ptr<MobilityModel> senderModelss;
Ptr<Ipv4> prev_m_ipv4;
int CountingHops;
double SumPPT;
Time PPT;
...
void
RreqHeader::SetPPT(Time ppt){
  PPT = ppt;
}

int
RreqHeader::GetCountingHops(){
  return CountingHops;
}
void
RreqHeader::SetCountingHops(int count){
  CountingHops = count;
}

double
RreqHeader::GetSumPPT(){
  return SumPPT;
}
void
RreqHeader::SetSumPPT(double ppt){
  SumPPT += ppt;
}

Ptr<Ipv4>
RreqHeader::GetPrevIpv4(){
  return prev_m_ipv4;
}
void
RreqHeader::SetPrevIpv4(Ptr<Ipv4> m_ipv4){
  prev_m_ipv4 = m_ipv4;
}
...
{
  static TypeId tid = TypeId ("ns3::ptrp::RreqHeader")
    .SetParent<Header> ()
    .SetGroupName ("ptrp")
...
static int64_t rttStartStampRrep;


void
RrepHeader::SetRTTstartRrep(int64_t startStampRrep){
  rttStartStampRrep = startStampRrep;
}

int64_t
RrepHeader::GetRTTstartRrep() const {
  return rttStartStampRrep;
}
...
{
  static TypeId tid = TypeId ("ns3::ptrp::RrepHeader")
    .SetParent<Header> ()
    .SetGroupName ("ptrp")
...
{
  static TypeId tid = TypeId ("ns3::ptrp::RrepAckHeader")
    .SetParent<Header> ()
    .SetGroupName ("ptrp")
...
{
  static TypeId tid = TypeId ("ns3::ptrp::RerrHeader")
    .SetParent<Header> ()
    .SetGroupName ("ptrp")
