 ...
 *
 * Based on
 *      NS-2 qosrp model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      qosrp-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/qosrp-UU
 *
...
#include "qosrp-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("qosrpRoutingProtocol");

namespace qosrp {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for qosrp control traffic
const uint32_t RoutingProtocol::qosrp_PORT = 654;

/**
* \ingroup qosrp
* \brief Tag used by qosrp implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::qosrp::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("qosrp")
...
{
  static TypeId tid = TypeId ("ns3::qosrp::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No qosrp interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // qosrp is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == qosrp_PORT)
                    {
                      // qosrp packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("qosrp does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvqosrp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), qosrp_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvqosrp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), qosrp_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("qosrp does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in qosrp operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when qosrp does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // qosrp needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available qosrp interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid qosrp source address not found");
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, qosrp_PORT));
...
RoutingProtocol::Recvqosrp (Ptr<Socket> socket)
{
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("qosrp message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (qosrpTYPE_RREP);
  packet->AddHeader (tHeader);
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (qosrpTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), qosrp_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (qosrpTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, qosrp_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (qosrpTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (qosrpTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, qosrp_PORT));
        }
    }
}
