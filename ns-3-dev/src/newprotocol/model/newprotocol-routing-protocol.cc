 ...
 *
 * Based on
 *      NS-2 newprotocol model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      newprotocol-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/newprotocol-UU
 *
...
#include "newprotocol-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("newprotocolRoutingProtocol");

namespace newprotocol {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for newprotocol control traffic
const uint32_t RoutingProtocol::newprotocol_PORT = 654;

/**
* \ingroup newprotocol
* \brief Tag used by newprotocol implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::newprotocol::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("newprotocol")
...
{
  static TypeId tid = TypeId ("ns3::newprotocol::RoutingProtocol")
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        ...
                        << ", newprotocol Routing table" << std::endl;
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No newprotocol interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // newprotocol is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == newprotocol_PORT)
                    {
                      // newprotocol packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("newprotocol does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvnewprotocol, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), newprotocol_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvnewprotocol, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), newprotocol_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("newprotocol does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in newprotocol operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when newprotocol does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // newprotocol needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available newprotocol interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid newprotocol source address not found");
...
{
  sumPPTjni = 0;  // May not need to reset to 0 here
  ...
  rreqHeader.SetSumPPT(0);
  rreqHeader.SetCountingHops(1);
  rreqHeader.SetPPT(Simulator::Now());
  ...
  // Send RREQ as subnet directed broadcast from each interface used by newprotocol
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (newprotocolTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, newprotocol_PORT));
...
void
RoutingProtocol::Recvnewprotocol (Ptr<Socket> socket)
{
...
//Find where neighbor is invoked and just pass the new neighbor list back and forth from there to make it work
  ...
  NS_LOG_DEBUG ("newprotocol node " << this << " received a newprotocol packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("newprotocol message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }

  senderFrameSize = packet->GetSize();
  switch (tHeader.Get ())
    {
    case newprotocolTYPE_RREQ:
      {
        RecvRequest (packet, receiver, sender);
        break;
      }
    case newprotocolTYPE_RREP:
      {
        RecvReply (packet, receiver, sender);
        break;
      }
    case newprotocolTYPE_RERR:
      {
        RecvError (packet, sender);
        break;
      }
    case newprotocolTYPE_RREP_ACK:
      {
        RecvReplyAck (sender);
        break;
      }
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
 ...
 // double Nni = rreqHeader.GetNeighborsLength();
  ...
  if(timerTd >= Simulator::Now ().GetSeconds()){
    ...
    //Ra = (CalculateUncoveredNodeSet(rreqHeader, rreqHeader.GetNeighborsLength())) / ns;
//double unused = distance+Hops+Dist+SoT+SFL+RFL+DR+TQE+Nni+Tci;
  ifstream stream;
  std::string const HOME = std::getenv("HOME") ? std::getenv("HOME") : ".";
  stringstream ss;
  ss << (m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0).GetLocal());
  ...
  RFL = RFL + (4 * ns);
    ...
    //Nni = rreqHeader.GetNeighborsLength();
    // Nni = 12;
//can divide by nni for num of hops?
//distance from prev node from the rreqJ
//can use this for RTT array:
...
//need distance calculated in packet
  ...
  //  cout << "SFL VS RFL  " << SFL << "  " << RFL << endl;
//cout << "covored neighbors: " << rreqHeader.GetCoveredNeighborsLength() /*<< endl*/;

  //cout << "RFL: " << RFL << "  vs. SFL " << SFL << endl;

//trying to read a specific nodes .txt then get that files size, add to RFL + distance x y doubles + int for counting sum(nvm the sum can probs be used from Nni or ns directly from the packet)
//instead of being an idiot and getting distence and all of Tci from each packet, just put a count up there then use that instead of countForAveragePPT
//if going by distance anyways mayube make something thats good for low distance topologies and one for large topologies

    cout << "countPPT = " << countForAveragePPT << " vs " << rreqHeader.GetCountingHops() << endl;
    
    sumPPTjni += PPTjni;
    rreqHeader.SetSumPPT(PPTjni);

    countForAveragePPT++;
    rreqHeader.SetCountingHops(1 + (rreqHeader.GetCountingHops()));
    ...
    sumTimeDifference += timeDifference + (Fc - Fc);
    ...
    //double CFni = timeDifference / (sumTimeDifference / countForAveragePPT);
    //CFni *= countForAveragePPT;
    //CFni *= rreqHeader.GetHopCount();

    double FPni = Ra * CFni/* * Fc*/;



    cout << "CFni = PPTjni(" << PPTjni << ") / sumPPT(" << sumPPTjni << ") / hops(" << rreqHeader.GetCountingHops() << endl;
    cout <<"Nni: " << Nni << " * CFni: " << CFni << " * Ra:" << Ra << "   =    FPni: " << FPni << "   vs. random: " << random;
    if(random <= FPni){
      //Continues to broadcast
      cout << "   ----IT BROADCASTED!----" << endl;
    } else {
      //Drops packet
      cout << "   ----dropped packet----" << endl;
      return;
    }
    //Find RTT for all RREP's and sort into array A
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (newprotocolTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), newprotocol_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (newprotocolTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), newprotocol_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (newprotocolTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, newprotocol_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (newprotocolTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (newprotocolTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, newprotocol_PORT));
        }
    }
}
