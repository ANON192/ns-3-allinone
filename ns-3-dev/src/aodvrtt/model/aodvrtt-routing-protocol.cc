 ...
 *
 * Based on
 *      NS-2 aodvrtt model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      aodvrtt-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/aodvrtt-UU
 *
...
#include "aodvrtt-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("aodvrttRoutingProtocol");

namespace aodvrtt {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for aodvrtt control traffic
const uint32_t RoutingProtocol::aodvrtt_PORT = 654;

/**
* \ingroup aodvrtt
* \brief Tag used by aodvrtt implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::aodvrtt::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("aodvrtt")
...
{
  static TypeId tid = TypeId ("ns3::aodvrtt::RoutingProtocol")
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        ...
                        << ", aodvrtt Routing table" << std::endl;
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No aodvrtt interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // aodvrtt is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == aodvrtt_PORT)
                    {
                      // aodvrtt packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("aodvrtt does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvaodvrtt, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), aodvrtt_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvaodvrtt, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), aodvrtt_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("aodvrtt does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in aodvrtt operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when aodvrtt does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // aodvrtt needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available aodvrtt interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid aodvrtt source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Send RREQ as subnet directed broadcast from each interface used by aodvrtt
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (aodvrttTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, aodvrtt_PORT));
...
void
RoutingProtocol::Recvaodvrtt (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("aodvrtt node " << this << " received a aodvrtt packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("aodvrtt message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
    case aodvrttTYPE_RREQ:
      {
        RecvRequest (packet, receiver, sender);
        break;
      }
    case aodvrttTYPE_RREP:
      {
        RecvReply (packet, receiver, sender);
        break;
      }
    case aodvrttTYPE_RERR:
      {
        RecvError (packet, sender);
        break;
      }
    case aodvrttTYPE_RREP_ACK:
      {
        RecvReplyAck (sender);
        break;
      }
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (aodvrttTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), aodvrtt_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (aodvrttTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), aodvrtt_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (aodvrttTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, aodvrtt_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (aodvrttTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
...
*/
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (aodvrttTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, aodvrtt_PORT));
        }
    }
}
