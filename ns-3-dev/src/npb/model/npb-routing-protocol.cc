 ...
 *
 * Based on
 *      NS-2 npb model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      npb-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/npb-UU
 *
...
#include "npb-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("npbRoutingProtocol");

namespace npb {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for npb control traffic
const uint32_t RoutingProtocol::npb_PORT = 654;

/**
* \ingroup npb
* \brief Tag used by npb implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::npb::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("npb")
...
{
  static TypeId tid = TypeId ("ns3::npb::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No npb interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // npb is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == npb_PORT)
                    {
                      // npb packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("npb does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvnpb, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), npb_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvnpb, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), npb_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("npb does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in npb operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when npb does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // npb needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available npb interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid npb source address not found");
...
RoutingProtocol::CalculateDkni (RreqHeader rreqHeader, int neighbors)
{
  ...
  double dkni = 0;
  double Ukni = CalculateUncoveredNodeSet(rreqHeader, Ni);
  double dMax = 0.128;
  double ani = 0;
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Send RREQ as subnet directed broadcast from each interface used by npb
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (npbTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, npb_PORT));
...
void
RoutingProtocol::Recvnpb (Ptr<Socket> socket)
{
...
//Find where neighbor is invoked and just pass the new neighbor list back and forth from there to make it work
  ...
  NS_LOG_DEBUG ("npb node " << this << " received a npb packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("npb message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
...
/*
//in order to listen and calculate at the same time do this:
void PeriodicCheck ()
{

  Simulator::Schedule (Seconds(1), &PeriodicCheck);
}

*/


  timerTd = Simulator::Now ().GetSeconds() + CalculateDkni(rreqHeader, rreqHeader.GetNeighborsLength());
//cout << CalculateDkni(rreqHeader, rreqHeader.GetNeighborsLength()) << endl;
 ...
 //if already exists and is duplicate, do the algorithm update/drop packet
  ...
  if(timerTd >= Simulator::Now ().GetSeconds()){
    ...
    Ra = (CalculateDkni(rreqHeader, rreqHeader.GetNeighborsLength())) / ns;

    if(rreqHeader.GetId() == rreqHeaderJ.GetId()){
...
*/
  ...
  if(timerTd >= Simulator::Now ().GetSeconds()){
    ...
    double Ckni = (CalculateUncoveredNodeSet(rreqHeader, rreqHeader.GetNeighborsLength())) / Nni;

    double Akni = (5.1774 * log(totalNodes)) / abs(Nni - CalculateUncoveredNodeSet(rreqHeader, rreqHeader.GetNeighborsLength()));

    double Pkni = Ckni * Akni;
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (npbTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), npb_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (npbTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), npb_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (npbTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, npb_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (npbTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (npbTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, npb_PORT));
        }
    }
}
