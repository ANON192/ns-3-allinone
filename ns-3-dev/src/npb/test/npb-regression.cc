 ...
 * \ingroup tests
 *
 * \brief ncpr regression test suite
 */
class ncprRegressionTestSuite : public TestSuite
{
public:
  ncprRegressionTestSuite () : TestSuite ("routing-ncpr-regression", SYSTEM)
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // General RREQ-RREP-RRER test case
    AddTestCase (new ChainRegressionTest ("ncpr-chain-regression-test"), TestCase::QUICK);
...
} g_ncprRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest (const char * const prefix, Time t, uint32_t size, Time arpAliveTimeout)
  : TestCase ("ncpr chain regression test"),
    m_nodes (0),
    m_prefix (prefix),
    m_time (t),
    m_size (size),
