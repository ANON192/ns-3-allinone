 ...
 *
 * Based on
 *      NS-2 ftaorp model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      ftaorp-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/ftaorp-UU
 *
...
#include "ftaorp-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ftaorpRoutingProtocol");

namespace ftaorp {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for ftaorp control traffic
const uint32_t RoutingProtocol::ftaorp_PORT = 654;

/**
* \ingroup ftaorp
* \brief Tag used by ftaorp implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::ftaorp::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("ftaorp")
...
{
  static TypeId tid = TypeId ("ns3::ftaorp::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No ftaorp interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // ftaorp is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == ftaorp_PORT)
                    {
                      // ftaorp packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("ftaorp does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvftaorp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), ftaorp_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvftaorp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), ftaorp_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("ftaorp does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in ftaorp operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when ftaorp does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // ftaorp needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available ftaorp interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid ftaorp source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Send RREQ as subnet directed broadcast from each interface used by ftaorp
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (ftaorpTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, ftaorp_PORT));
...
void
RoutingProtocol::Recvftaorp (Ptr<Socket> socket)
{
...
//Find where neighbor is invoked and just pass the new neighbor list back and forth from there to make it work
  ...
  NS_LOG_DEBUG ("ftaorp node " << this << " received a ftaorp packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("ftaorp message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
    case ftaorpTYPE_RREQ:
      {
        RecvRequest (packet, receiver, sender);
        break;
      }
    case ftaorpTYPE_RREP:
      {
        RecvReply (packet, receiver, sender);
        break;
      }
    case ftaorpTYPE_RERR:
      {
        RecvError (packet, sender);
        break;
      }
    case ftaorpTYPE_RREP_ACK:
      {
        RecvReplyAck (sender);
        break;
      }
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (ftaorpTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), ftaorp_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (ftaorpTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), ftaorp_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (ftaorpTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, ftaorp_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (ftaorpTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (ftaorpTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, ftaorp_PORT));
        }
    }
}
