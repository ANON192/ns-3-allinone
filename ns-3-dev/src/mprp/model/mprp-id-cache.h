...
#ifndef mprp_ID_CACHE_H
#define mprp_ID_CACHE_H
...
public:
  mprpRoute() {}
  mprpRoute(Ipv4Address nextHop, Ipv4Address lastHop = 0) : m_nextHop (nextHop) , m_lastHop (lastHop) {}
...
public:
  ...
  struct UniqueId
  {
    ...
    Time m_expire;
    int	count;
    std::vector<mprpRoute> m_reversePathList;     // List of reverse paths used for forwarding replies
    std::vector<mprpRoute> m_forwardPathList;     // List of forward paths advertised already
    void ReversePathInsert (Ipv4Address nextHop, Ipv4Address lastHop = 0);
    bool ReversePathLookup (Ipv4Address nextHop, mprpRoute & rt, Ipv4Address lastHop = 0);
    void ForwardPathInsert (Ipv4Address nextHop, Ipv4Address lastHop = 0);
    bool ForwardPathLookup (Ipv4Address nextHop, mprpRoute & rt, Ipv4Address lastHop = 0);
  };
