...
{
  mprpRoute* route = new mprpRoute (nextHop, lastHop); 
  m_reversePathList.push_back (*route);
}

bool 
IdCache::UniqueId::ReversePathLookup (Ipv4Address nextHop, mprpRoute & rt, Ipv4Address lastHop)
{
  for (std::vector<mprpRoute>::iterator i = m_reversePathList.begin(); i != m_reversePathList.end(); ++i) 
    {
...
bool
IdCache::UniqueId::ForwardPathLookup (Ipv4Address nextHop, mprpRoute & rt, Ipv4Address lastHop)
{
  for (std::vector<mprpRoute>::iterator i = m_forwardPathList.begin(); i != m_forwardPathList.end(); ++i) 
    {
      if ( ( i->GetNextHop () == nextHop) && ( i->GetLastHop () == lastHop) )
        {
	      rt = *i;
