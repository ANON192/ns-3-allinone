#ifndef MPRP_RTABLE_H
#define MPRP_RTABLE_H
...
public:
  RouteItem(Ptr<NetDevice> dev=0, Ipv4Address dst = Ipv4Address(), Ipv4Address src=Ipv4Address(), 
               Ipv4Address nextHop = Ipv4Address(),uint64_t rtt=0,RouteFlags flag = INIT);  
 ...
 ~RouteItem(){};
  ...
  void SetPreviousHop(Ipv4Address prev)
  {
    m_prevHop = prev;
  }

  Ipv4Address GetPreviousHop()
  {
    return m_prevHop;
  }
  ...
  uint64_t ToNow()
  {
    return (Simulator::Now()-m_stablisTime).GetMilliSeconds();
  }
  void SetRttToNow()
  {
    m_rtt = ToNow();
...
public:
  ...
  bool FindInitRouteItem(RouteItem & initRouteItem);
  bool FindWithSrcFlag(Ipv4Address src,RouteItem & rt, RouteFlags f=VALID);
  bool FindWithPrev(Ipv4Address prevHop, RouteItem & rt );
...
/**
 * \ingroup mprp
 * \brief The Routing table used by mprp protocol
 */
class RoutingTable
{
public:
