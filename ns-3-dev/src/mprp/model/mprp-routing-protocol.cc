...
#include "mprp-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("MprpRoutingProtocol");

namespace mprp {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for mprp control traffic
const uint32_t RoutingProtocol::MPRP_PORT = 654;

/**
* \ingroup mprp
* \brief Tag used by mprp implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::mprp::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("Mprp")
...
{
  static TypeId tid = TypeId ("ns3::mprp::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No mprp interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
      ...
      if (!result || ((rt.GetFlag () != IN_SEARCH) && result))
        {
          ...
          //std::cout<<"Send new RREQ for outbound packet to " << header.GetDestination ()<<std::endl;
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  Ipv4Address dst = header.GetDestination ();//dest
  Ipv4Address origin = header.GetSource ();//sender
  ...
  // mprp is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == MPRP_PORT)
                    {
                      // mprp packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      if (toDst.GetFlag () == VALID && !toDst.IsEmptyRoute())
        {
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  RouteItem nRt(m_lo, Ipv4Address::GetLoopback () ,
                      m_ipv4->GetAddress (0, 0).GetLocal (),
                      Ipv4Address::GetLoopback (),0,VALID);
  nRt.SetPreviousHop(m_ipv4->GetAddress (0, 0).GetLocal ());
  rt.AddRouteItem(nRt);
  m_routingTable.AddRoute (rt,loopback);
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("mprp does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvMprp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), MPRP_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvMprp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), MPRP_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
  ...
  RouteItem nRt(dev,iface.GetBroadcast (),iface.GetBroadcast (),iface.GetBroadcast (),0,VALID);
  NS_LOG_UNCOND(iface.GetBroadcast ());
  rt.AddRouteItem (nRt);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("MPRP does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in MPRP operation");
    }
}
...
void RoutingProtocol::PrintIface ()
{
  std::string str = "";
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when MPRP does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // MPRP needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available MPRP interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid MPRP source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  if (m_routingTable.LookupRoute (dst, rt))
    {
      ...
      RouteItem newRt(0,dst,Ipv4Address(),Ipv4Address(),0,INIT);
      rt.AddRouteItem(newRt);
      m_routingTable.Update (rt,dst);
    }
  else
    {
      rreqHeader.SetUnknownSeqno (true);
      ...
      RouteItem newRt(dev,dst,Ipv4Address(),Ipv4Address(),0,INIT);
      newRt.SetPreviousHop(Ipv4Address());
      newEntry.AddRouteItem(newRt);
      ...
      m_routingTable.AddRoute (newEntry,dst);
    }
  ...
  // Send RREQ as subnet directed broadcast from each interface used by MPRP
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      //std::cout<< rreqHeader.GetOrigin() <<" Rreq for "<< dst<<std::endl;
      ...
      TypeHeader tHeader (mprpTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, MPRP_PORT));
...
void
RoutingProtocol::RecvMprp (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("MPRP node " << this << " received a MPRP packet from " << sender << " to " << receiver);
  
  //UpdateRouteToNeighbor (sender, receiver);
  TypeHeader tHeader (mprpTYPE_RREQ);
  packet->RemoveHeader (tHeader);
  //std::cout<<"Rcv MPRP: ";
/*  std::cout<<"MPRP node " << socket->GetNode()->GetId() << " received a MPRP packet from " 
      << sender << " to " << receiver<< " h: " << tHeader.Get()<<std::endl;*/

  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("MPRP message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
    ...
    case mprpTYPE_RREP_ACK:
      {
        //std::cout<<"!!!!!!rep_ACK!!!!!!"<<std::endl;
        RecvReplyAck (sender);
        break;
      }
    }
...
RoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver)
{
  ...
  if (!m_routingTable.LookupRoute (sender, toNeighbor))
  ...
  else
    {
      ...
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
      if(!toNeighbor.IsValidInterFace())
      {
        toNeighbor.SetInterface(m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0));
      }
      if(toNeighbor.IsEmptyRoute()){
        
        RouteItem newRt(dev,sender,toNeighbor.GetInterface().GetLocal(),sender,
                        0,VALID);
        toNeighbor.AddRouteItem(newRt);
        
        m_routingTable.Update (toNeighbor);
      }
      if (toNeighbor.GetValidSeqNo () && (toNeighbor.GetHop () == 1) && (toNeighbor.GetOutputDevice (sender) == dev))
        {
...
/* 
 * reciver => current node IP
 * src => previous Node IP 
 * origin => issuer of rreq
 * dst => Final destination of request
 * */
void
RoutingProtocol::
RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  NS_LOG_UNCOND(receiver << " received req from "<< origin << " via "<< src<< " to "<< rreqHeader.GetDst ());
  /*
   *  Node checks to determine whether it has received a RREQ with the same Originator IP Address and RREQ ID.
   *  If such a RREQ has been received, the node silently discards the newly received RREQ.
   * 
   *  node disjoint
   *  should not be applied on destination node in case of MPRP
   */
  if (m_rreqIdCache.IsDuplicate (origin, id) && !IsMyOwnAddress (rreqHeader.GetDst ()))
    {
      NS_LOG_DEBUG ("Ignoring RREQ due to duplicate");
      //std::cout<<origin<<" Ignoring RREQ due to duplicate "<<id<<std::endl;
      return;
    }
  ...
  if (IsMyOwnAddress (rreqHeader.GetDst ()))
    {
      IncRoutingPacketCount();
      if(!m_routingTable.LookupRoute(origin,toOrigin))
      {
        Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
        RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ receiver, /*know seqno=*/ true, /*seqno=*/rreqHeader.GetOriginSeqno(),
                                              /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                                              /*hops=*/ 1, /*next hop=*/ src, /*lifetime=*/ m_activeRouteTimeout);
        RouteItem newRt(dev,origin,receiver,src,0,WAIT_FOR_REPLY);
        newRt.SetPreviousHop(Ipv4Address());
        newEntry.AddRouteItem(newRt);
        m_routingTable.AddRoute (newEntry,origin);
        SendReply (rreqHeader, newEntry,newRt);
      }
      else
      {
        RouteItem _temp;
        if(!toOrigin.FindWithPrev(src,_temp))
        {
          Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
          RouteItem newRt(dev,origin,receiver,Ipv4Address(),0,WAIT_FOR_REPLY);
          toOrigin.SetInterface(m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver),0));
          newRt.SetPreviousHop(Ipv4Address());
          toOrigin.AddRouteItem(newRt);
          m_routingTable.AddRoute (toOrigin,origin);
          rreqHeader.SetDstSeqno(toOrigin.GetSeqNo());
          rreqHeader.SetUnknownSeqno (false);
          m_routingTable.Update(toOrigin,origin);
          SendReply (rreqHeader, toOrigin,newRt);
        }
      }
      return;
    }
  ...
  {
    Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
    RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ src, /*know seqno=*/ false, /*seqno=*/ 0,
                                              /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                                              /*hops=*/ 1, /*next hop=*/ src, /*lifetime=*/ m_activeRouteTimeout);
    RouteItem newRt(dev,dst,origin,Ipv4Address(),0,WAIT_FOR_REPLY);
    newRt.SetPreviousHop(src);
    m_routingTable.AddRoute (newEntry,dst);
  }
  else
  {
    RouteItem _temp;
    if(!toDst.FindWithPrev(src,_temp))
    {
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
      RouteItem newRt(dev,dst,origin,Ipv4Address(),0,WAIT_FOR_REPLY);
      newRt.SetPreviousHop(src);
      toDst.AddRouteItem(newRt);
      m_routingTable.Update(toDst,dst);
    }
  }
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      ...
      /* if ((rreqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (rreqHeader.GetDstSeqno ()) >= 0))
          && toDst.GetValidSeqNo () )
        {
          if (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID && !toDst.IsEmptyRoute())
            {
              m_routingTable.LookupRoute (origin, toOrigin);
              if(toOrigin.IsEmptyRoute() || toDst.IsEmptyRoute())
              {
                std::cout<<"";
              }
                            ...
                            RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (mprpTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), MPRP_PORT));
}
void
RoutingProtocol::SendReply (RreqHeader const & rreqHeader,
                            RoutingTableEntry const & toOrigin,
                            RouteItem & toNext)
{
  ...
  RrepHeader rrepHeader ( /*prefixSize=*/ 0, /*hops=*/ 0, /*dst=*/ rreqHeader.GetDst (),
                                          /*dstSeqNo=*/ m_seqNo, 
                                          /*origin=*/ toNext.GetDestination (),
                                          /*lifeTime=*/ m_myRouteTimeout);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (toNext.GetHops ());
  packet->AddPacketTag (tag);
  packet->AddHeader (rrepHeader);
  TypeHeader tHeader (mprpTYPE_RREP);
  packet->AddHeader (tHeader);
  ...
  // std::cout<<"reply to:"<<toNext.GetDestination ()<<" via: "<<toNext.GetNextHop()<<std::endl;
  NS_LOG_DEBUG("reply to:"<<toNext.GetDestination ()<<" via: "<<toNext.GetNextHop()<<" from "<< rrepHeader.GetDst() << " hop "<< toNext.GetHops());
  socket->SendTo (packet, 0, InetSocketAddress (toNext.GetNextHop (), MPRP_PORT));
}

void 
RoutingProtocol::SendReplyAck(RrepHeader const & rrepHeader, RoutingTableEntry const & toOrigin, RouteItem & toNext)
{
  
  RrepHeader rrepHeaderAck ( /*prefixSize=*/ 0, /*hops=*/ 0, /*dst=*/ rrepHeader.GetOrigin (),
                                          /*dstSeqNo=*/ m_seqNo, 
                                          /*origin=*/ toNext.GetDestination (),
                                          /*lifeTime=*/ m_myRouteTimeout);
  Ptr<Packet> packet = Create<Packet> ();
  ...
  tag.SetTtl (toNext.GetHops ());
  packet->AddPacketTag (tag);
  packet->AddHeader (rrepHeaderAck);
  TypeHeader tHeader (mprpTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (mprpTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), MPRP_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (mprpTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, MPRP_PORT));
}
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  uint8_t hops = rrepHeader.GetHopCount () + 1;
  rrepHeader.SetHopCount (hops);
  ...
  if (IsMyOwnAddress (rrepHeader.GetOrigin ()))
    {
      NS_LOG_UNCOND("rev reply at "<< receiver << " via " << sender << 
        " to " << rrepHeader.GetOrigin() << // req sender
        " from " << rrepHeader.GetDst()<< // rep sender
        " hop "<< uint32_t(hops) );
      IncRoutingPacketCount();
      ...
      RouteItem init;
      if(toDst.FindInitRouteItem(init))
      {
        RouteItem temp;
        if(toDst.FindRoute(sender,temp))
        {
          if(temp.ToNow()>=m_myRouteTimeout || temp.GetHops() >= init.ToNow())
          {
            toDst.RemoveNextHopRoute(sender);
            Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
            RouteItem newRt(dev,dst,receiver,sender,init.ToNow(),VALID);
            toDst.AddRouteItem(newRt);
            m_routingTable.Update(toDst,dst);
          }
          else
      ...
      m_routingTable.Update (toDst,dst);
      RouteItem rt;
      toDst.FindRoute(sender,rt);
      SendReplyAck(rrepHeader,toDst,rt);      
...
RouteItem rt;
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      ...
      if(toDst.FindWithSrcFlag(origin,rt,WAIT_FOR_REPLY))
      {
        rt.SetRttToNow();
        rt.ReNewInitTime();
        rt.SetFlag(VALID);
      ...
      if(!m_routingTable.LookupRoute (origin, toOrigin))
      {
        ...
        RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ origin, /*know seqno=*/ true, /*seqno=*/rrepHeader.GetDstSeqno(),
                                              /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                                              /*hops=*/ 1, /*next hop=*/ Ipv4Address(), /*lifetime=*/ m_activeRouteTimeout);
        RouteItem newRt(dev,origin,dst,rt.GetPreviousHop(),0,WAIT_FOR_REPLY);
        newRt.SetPreviousHop(sender);
        newEntry.AddRouteItem(newRt);
        m_routingTable.AddRoute (newEntry,origin);        
      }
      else
      {
        if(!toOrigin.FindWithPrev(sender,rt))
        {
          RouteItem newRt(dev,origin,dst,Ipv4Address(),0,WAIT_FOR_REPLY);
          toOrigin.AddRouteItem(newRt);
        }
        m_routingTable.Update(toOrigin,origin);
      }
  ...
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (rt.GetPreviousHop (), MPRP_PORT));
}
...
RoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
  ...
  if (!m_routingTable.LookupRoute (rrepHeader.GetDst (), toNeighbor))
  ...
  else
    {
      ...
      RouteItem toNrb;
      if(!toNeighbor.FindRoute(rrepHeader.GetDst (), toNrb)){
        Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
        
        RouteItem newRt(dev,rrepHeader.GetOrigin(),
                        rrepHeader.GetDst (),toNeighbor.GetInterface().GetLocal(),
                        0,WAIT_FOR_REPLY);
        toNeighbor.AddRouteItem(newRt);
        m_routingTable.Update(toNeighbor,dst);
      }
      else
      {
        toNrb.SetHops(1);
        toNrb.SetHops (1);
        toNrb.SetNextHop (rrepHeader.GetDst ());
        toNrb.SetDestination(rrepHeader.GetDst ());
        toNrb.SetOutputDevice (m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver)));
        toNeighbor.Update(toNrb);
      }
...
RoutingProtocol::RecvError (Ptr<Packet> p, Ipv4Address src )
{
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (mprpTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
  ...
  m_routingTable.InvalidateRoutesWithDst (dstWithNextHopSrc);
}
...
RoutingProtocol::RouteRequestTimerExpire (Ipv4Address dst)
{
  NS_LOG_LOGIC (this);
  RoutingTableEntry toDst;
  if (m_routingTable.LookupValidRoute (dst, toDst) && !toDst.IsEmptyRoute())
    {
      SendPacketFromQueue (dst, toDst.GetRoute ());
  ...
  while (m_queue.Dequeue (dst, queueEntry))
    {
      ...
      //std::cout<<"p send src: "<<route->GetSource ()<<std::endl;
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (mprpTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, MPRP_PORT));
        }
    }
}
