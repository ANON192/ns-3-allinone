...
NS_LOG_COMPONENT_DEFINE ("mprpRoutingTable");
...
RouteItem::RouteItem(Ptr<NetDevice> dev, Ipv4Address dst, Ipv4Address src, 
     Ipv4Address nextHop, uint64_t rtt,RouteFlags flag)
    {
...
bool RoutingTableEntry::FindInitRouteItem(RouteItem & irt)
{
  if(m_rlist.empty()){
    return false;
  }
...
bool RoutingTableEntry::FindWithPrev(Ipv4Address prevHop,  RouteItem & rt ){
  if(m_rlist.empty()){
    return false;
  }
  for(std::list<RouteItem>::iterator it=m_rlist.begin();
        it != m_rlist.end();++it){
  if(it->GetPreviousHop() == prevHop){
    rt = it->getMe();    
    return true;
  }
  }
...
bool
RoutingTableEntry::FindWithSrcFlag(Ipv4Address src,RouteItem & rt, RouteFlags f)
{
  NS_LOG_FUNCTION (this);
  if (m_rlist.empty ())
    {
  ...
  for(std::list<RouteItem>::iterator it=m_rlist.begin();
        it != m_rlist.end();++it){
  if(it->GetFlag() == f && it->GetSource() == src){
    rt = it->getMe();    
    return true;
  }
  }
