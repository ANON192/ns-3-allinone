 ...
 */
#include "ns3/test.h"
#include "ns3/ncpr-neighbor.h"
#include "ns3/ncpr-packet.h"
#include "ns3/ncpr-rqueue.h"
#include "ns3/ncpr-rtable.h"
...
{
  TypeHeaderTest () : TestCase ("ncpr TypeHeader")
  {
  }
  virtual void DoRun ()
  {
    TypeHeader h (ncprTYPE_RREQ);
    NS_TEST_EXPECT_MSG_EQ (h.IsValid (), true, "Default header is valid");
    NS_TEST_EXPECT_MSG_EQ (h.Get (), ncprTYPE_RREQ, "Default header is RREQ");

    Ptr<Packet> p = Create<Packet> ();
    p->AddHeader (h);
    TypeHeader h2 (ncprTYPE_RREP);
...
{
  RreqHeaderTest () : TestCase ("ncpr RREQ")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepHeaderTest () : TestCase ("ncpr RREP")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepAckHeaderTest () : TestCase ("ncpr RREP-ACK")
  {
  }
  virtual void DoRun ()
  {
...
{
  RerrHeaderTest () : TestCase ("ncpr RERR")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for ncpr routing table entry
 */
...
struct ncprRqueueTest : public TestCase
{
  ncprRqueueTest () : TestCase ("Rqueue"),
                      q (64, Seconds (30))
  {
  }
  virtual void DoRun ();
...
ncprRqueueTest::DoRun ()
{
  ...
  h.SetDestination (Ipv4Address ("1.2.3.4"));
  h.SetSource (Ipv4Address ("4.3.2.1"));
  Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback (&ncprRqueueTest::Unicast, this);
  Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback (&ncprRqueueTest::Error, this);
  ...
  Simulator::Schedule (q.GetQueueTimeout () + Seconds (1), &ncprRqueueTest::CheckTimeout, this);
 ...
 * \brief Unit test for ncpr routing table entry
 */
struct ncprRtableEntryTest : public TestCase
{
  ncprRtableEntryTest () : TestCase ("RtableEntry")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for ncpr routing table
 */
struct ncprRtableTest : public TestCase
{
  ncprRtableTest () : TestCase ("Rtable")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief ncpr test suite
 */
class ncprTestSuite : public TestSuite
{
public:
  ncprTestSuite () : TestSuite ("routing-ncpr", UNIT)
  {
    ...
    AddTestCase (new ncprRqueueTest, TestCase::QUICK);
    AddTestCase (new ncprRtableEntryTest, TestCase::QUICK);
    AddTestCase (new ncprRtableTest, TestCase::QUICK);
  }
} g_ncprTestSuite; ///< the test suite
