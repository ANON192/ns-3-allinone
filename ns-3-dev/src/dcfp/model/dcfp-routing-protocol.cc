 ...
 *
 * Based on
 *      NS-2 dcfp model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      dcfp-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/dcfp-UU
 *
...
#include "dcfp-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("dcfpRoutingProtocol");

namespace dcfp {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for dcfp control traffic
const uint32_t RoutingProtocol::dcfp_PORT = 654;

/**
* \ingroup dcfp
* \brief Tag used by dcfp implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::dcfp::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("dcfp")
...
{
  static TypeId tid = TypeId ("ns3::dcfp::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No dcfp interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // dcfp is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == dcfp_PORT)
                    {
                      // dcfp packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("dcfp does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvdcfp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), dcfp_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvdcfp, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), dcfp_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("dcfp does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in dcfp operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when dcfp does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // dcfp needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available dcfp interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid dcfp source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Send RREQ as subnet directed broadcast from each interface used by dcfp
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (dcfpTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, dcfp_PORT));
...
//list<Ipv4Address>NodesCovoredNeighborList={};
/*
Ipv4Address
RoutingProtocol::NodesGetCoveredNeighbors ()
{
  return NodesCovoredNeighborList.front();
}

int
RoutingProtocol::NodesGetCoveredNeighborsLength ()
{
  return NodesCovoredNeighborList.size();
}

void
RoutingProtocol::NodesAddCoveredNeighbors (Ipv4Address p)
{
  NodesCovoredNeighborList.push_front(p); 
}*/

void
RoutingProtocol::Recvdcfp (Ptr<Socket> socket)
{
...
//Find where neighbor is invoked and just pass the new neighbor list back and forth from there to make it work
  ...
  NS_LOG_DEBUG ("dcfp node " << this << " received a dcfp packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("dcfp message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
 ...
 //if already exists and is duplicate, do the algorithm update/drop packet
  ...
  istream.close();
  //cout << " ns = " << ns << endl;
//find out if calculateuncofrofneroNODESET needs ns nni or rreq neighbors
  
  double a = 8.796112, b = 37.29479, c = 7.229256, d = 11.26162, m = 5.094965;
  double Ra = (CalculateUncoveredNodeSet(rreqHeader, rreqHeader.GetCoveredNeighborsLength())) / Nni;
  double random = ((double) rand() / (RAND_MAX));
  
  double NBni = 1 + pow((Nni / c), b);
  double DCFni = d + (a - d / pow(abs(NBni), m));

  if(DCFni > 1){
    DCFni = 1;
  }

  double DAFni = DCFni / Nni;
  double FPni = DAFni * Ra;
...
/*
  if(timerTd >= Simulator::Now ().GetSeconds()){
    double Ra = (CalculateUncoveredNodeSet(rreqHeader, rreqHeader.GetNeighborsLength())) / rreqHeader.GetNeighborsLength();
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (dcfpTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), dcfp_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (dcfpTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), dcfp_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (dcfpTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, dcfp_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (dcfpTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (dcfpTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, dcfp_PORT));
        }
    }
}
