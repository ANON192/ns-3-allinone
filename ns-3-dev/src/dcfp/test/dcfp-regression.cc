 ...
 * \ingroup tests
 *
 * \brief dcfp regression test suite
 */
class dcfpRegressionTestSuite : public TestSuite
{
public:
  dcfpRegressionTestSuite () : TestSuite ("routing-dcfp-regression", SYSTEM)
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // General RREQ-RREP-RRER test case
    AddTestCase (new ChainRegressionTest ("dcfp-chain-regression-test"), TestCase::QUICK);
...
} g_dcfpRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest (const char * const prefix, Time t, uint32_t size, Time arpAliveTimeout)
  : TestCase ("dcfp chain regression test"),
    m_nodes (0),
    m_prefix (prefix),
    m_time (t),
    m_size (size),
...
ChainRegressionTest::CreateDevices ()
{
  ...
  // 2. Setup TCP/IP & dcfp
  dcfpHelper dcfp; // Use default parameters here
  InternetStackHelper internetStack;
  internetStack.SetRoutingHelper (dcfp);
  ...
  streamsUsed += dcfp.AssignStreams (*m_nodes, streamsUsed);
  // dcfp uses m_size more streams
