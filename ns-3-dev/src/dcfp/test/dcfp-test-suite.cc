 ...
 */
#include "ns3/test.h"
#include "ns3/dcfp-neighbor.h"
#include "ns3/dcfp-packet.h"
#include "ns3/dcfp-rqueue.h"
#include "ns3/dcfp-rtable.h"
...
{
  TypeHeaderTest () : TestCase ("dcfp TypeHeader")
  {
  }
  virtual void DoRun ()
  {
    TypeHeader h (dcfpTYPE_RREQ);
    NS_TEST_EXPECT_MSG_EQ (h.IsValid (), true, "Default header is valid");
    NS_TEST_EXPECT_MSG_EQ (h.Get (), dcfpTYPE_RREQ, "Default header is RREQ");

    Ptr<Packet> p = Create<Packet> ();
    p->AddHeader (h);
    TypeHeader h2 (dcfpTYPE_RREP);
...
{
  RreqHeaderTest () : TestCase ("dcfp RREQ")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepHeaderTest () : TestCase ("dcfp RREP")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepAckHeaderTest () : TestCase ("dcfp RREP-ACK")
  {
  }
  virtual void DoRun ()
  {
...
{
  RerrHeaderTest () : TestCase ("dcfp RERR")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for dcfp routing table entry
 */
...
struct dcfpRqueueTest : public TestCase
{
  dcfpRqueueTest () : TestCase ("Rqueue"),
                      q (64, Seconds (30))
  {
  }
  virtual void DoRun ();
...
dcfpRqueueTest::DoRun ()
{
  ...
  h.SetDestination (Ipv4Address ("1.2.3.4"));
  h.SetSource (Ipv4Address ("4.3.2.1"));
  Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback (&dcfpRqueueTest::Unicast, this);
  Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback (&dcfpRqueueTest::Error, this);
  ...
  Simulator::Schedule (q.GetQueueTimeout () + Seconds (1), &dcfpRqueueTest::CheckTimeout, this);
 ...
 * \brief Unit test for dcfp routing table entry
 */
struct dcfpRtableEntryTest : public TestCase
{
  dcfpRtableEntryTest () : TestCase ("RtableEntry")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for dcfp routing table
 */
struct dcfpRtableTest : public TestCase
{
  dcfpRtableTest () : TestCase ("Rtable")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief dcfp test suite
 */
class dcfpTestSuite : public TestSuite
{
public:
  dcfpTestSuite () : TestSuite ("routing-dcfp", UNIT)
  {
    ...
    AddTestCase (new dcfpRqueueTest, TestCase::QUICK);
    AddTestCase (new dcfpRtableEntryTest, TestCase::QUICK);
    AddTestCase (new dcfpRtableTest, TestCase::QUICK);
  }
} g_dcfpTestSuite; ///< the test suite
