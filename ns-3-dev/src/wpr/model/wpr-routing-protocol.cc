 ...
 *
 * Based on
 *      NS-2 wpr model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      wpr-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/wpr-UU
 *
...
const uint32_t RoutingProtocol::wpr_PORT = 654;

/**
* \ingroup wpr
* \brief Tag used by wpr implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::wpr::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("wpr")
...
list<RoutingTableEntry> listRTT;
RreqHeader tempswap;
int m_broadcastCount = 0;
Ptr<MobilityModel> senderModel;
double senderFrameSize = 0;
double static sumPPTjni = 0;
double countForAveragePPT = 0;
  ...
  if (result)
    {
      ...
      if (!result || ((rt.GetFlag () != IN_SEARCH) && result))
        {
          //flag here choose from rtt array list here
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // wpr is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == wpr_PORT)
                    {
                      // wpr packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("wpr does not work with more then one address per each interface.");
    }
  ...
  socket->Bind (InetSocketAddress (iface.GetLocal (), wpr_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvwpr, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), wpr_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("wpr does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in wpr operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when wpr does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // wpr needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available wpr interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid wpr source address not found");
...
{  
  ...
  senderModel = (m_ipv4->GetObject<Node> ())->GetObject<MobilityModel>();
  ...
  int64_t temp = 0;
  temp = Simulator::Now().GetMilliSeconds();

  rreqHeader.SetRTTstart(temp);
  ...
  // Send RREQ as subnet directed broadcast from each interface used by wpr
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (wprTYPE_RREQ);
...
RoutingProtocol::Recvwpr (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("wpr node " << this << " received a wpr packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("wpr message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
 ...
 //if already exists and is duplicate, do the algorithm update/drop packet
  ...
  //std::string const HOME = std::getenv("HOME") ? std::getenv("HOME") : ".";
  stringstream ss2;
  ss2 << (m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0).GetLocal());
  string nodeName = ss2.str();
  stream.open(HOME + path + nodeName + ".txt", ios::in);
  ...
  //comeback here and figure out when to .SetNeighborsLength() and replace since nni never gets smaller just grows
    ...
    Ptr<MobilityModel> recieverModel = (m_ipv4->GetObject<Node> ())->GetObject<MobilityModel>();


    double distance = recieverModel->GetDistanceFrom (senderModel);
    if(rreqHeader.GetHopCount() == 1){
      //cout << "First hop." << endl;
      //rreqHeader.SetCountingHops(1);

    } else {
      distance = (distance / rreqHeader.GetHopCount());
    ...
    double Tci = ((Hops * Dist) / SoT) + ((SFL + RFL) / DR) + (TQE * Hops);
    double PPTjni = Tci;

    //cout << " ((Hops * Dist) / SoT) + ((SFL + RFL) / DR) + (TQE * Hops)" << endl << " ((1 * " << Dist << ") / 300000000) + ((23 + " << RFL << ") / 250000)+(0.002*1)" << endl;
  ...
  Time packetStamp = rreqHeader.GetPPT();
  Time now = Simulator::Now();
  //cout << "Time difference is: " << now.GetMilliSeconds() - packetStamp.GetMilliSeconds() << "   Timestamp: " << packetStamp.GetMilliSeconds() << " VS NOW: " << now.GetMilliSeconds() << endl;
  double timeDifference = now.GetMilliSeconds() - packetStamp.GetMilliSeconds();
  sumTimeDifference += timeDifference;

  //PPTjni = timeDifference;
  //sumPPTjni = sumTimeDifference;
  //Nni = rreqHeader.GetCountingHops();
  //double CFni = PPTjni / (rreqHeader.GetSumPPT() / rreqHeader.GetHopCount());

  double CFni = PPTjni / (sumPPTjni / countForAveragePPT);
...
/*
  if(timerTd >= Simulator::Now ().GetSeconds()){
    ...
    //distance/250 = percentage of distance of prev hop to transmission range default setting
    normalizer = CFni + (distance/250)/ sqrt(CalculateUncoveredNodeSet(rreqHeader, rreqHeader.GetNeighborsLength()));
    //normalizer = sqrt(PPTjni) / 1;
    //normalizer = 1 / sqrt(ns);
    //normalizer = 1 / log10(Nni);
    //Nni = 12;
    //normalizer /= Nni;

    double Fc = Nc / Nni;
    double Pre = (Ra * Fc) / (normalizer * 1);
    //Pre *= CFni;
    if(totalNodes <= 100){
      Pre += 0.25;
    }
    ...
    //double Pre = (Ra * Fc) * normalizer + (Fc - Fc);
    //cout << Simulator::Now ().GetSeconds() << " Timer: " << timerTd << endl;
    cout << "normalizer = " << normalizer << "  count = " << countForAveragePPT << " Fc = " << Fc << "  log10(ns) = " << log10(ns) << " CFni = " << CFni << endl;


    double random = ((double) rand() / (RAND_MAX));

    //unused
    if(CFni - CFni + Fc - Fc + PPTjni - PPTjni){};
    ...
    stream2 << "RA: " << Ra << " Distance: " << distance << " RFL: " << RFL << endl;
    stream2.close();
...
//Initializations
double a = 0, b = 0, c = 0, d = 0, r = 0, vi = 0, thetai = 0, vj = 0, thetaj = 0, xi = 0, yi = 0, xj = 0, yj = 0, xdif = 0, ydif = 0, radi = 0, radj = 0, qd = 0, cd = 0, td = 0, linkDelayVarience = 0, preLinkDelay = 0, Vw = 0, thetad = 0, Beta = 0, averageLd = 0, averageLdcount = 0, averageLdsum = 0, varphi = 0, RSFj = 0, probabilityStableLink = 0, probabilityUnstableLink = 0, Ys = 0, Djdn = 0;



// vi = senderModel->GetVelocity();
// vj = recieverModel->GetVelocity();

// Vector vecti = senderModel->DoGetPosition();
// xi = vecti.x();
// yi = vecti.y();
// Vector vectj = recieverModel->DoGetPosition();
// xj = vecti.x();
// yj = vecti.y();
...
// //Angle using trignometric properties in radians
// radi = Math.atan2(yi, xi);
// radj = Math.atan2(yj, xj);

// //Converting radians to degrees
// thetai = radi * (180 / Math.PI);
// thetaj = radj * (180 / Math.PI);
...
LET = - ( ( (a*b) + (c*d) ) + sqrt( (pow(a, 2) + (pow(c, 2) ) ) * pow(r, 2) - pow((a*d) - (b*c), 2) ) ) / (pow(a, 2) + pow(c, 2)); 

ld = qd + cd + td;
averageLd = averageLdsum / averageLdcount;

//Link Delay Varience
thetad = averageLd - ld;

//Inverse Weighted Function
Vw = (a * (1 / LET)) + (Beta * (thetad * a)) + Beta;

//Updated Link Stability
Ys = probabilityStableLink / (probabilityStableLink + probabilityUnstableLink);

//Reinitiating detouring neighbor denoted as RSF
RSFj = (varphi * Djdn) + ((1-varphi) * LET);



// ConstantVelocityHelper velocityHelper;
// velocityHelper.GetVelocity();
// //Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));


    //unused
    if(CFni + PPTjni + Nc + Ra + Nni+a+b+c+d+r+vi+vj+yi+yj+thetaj+thetai+xi+xj+ld+LET+xdif+ydif+linkDelayVarience+preLinkDelay+Djdn+radi+radj+Ys+RSFj+Vw){};
  // Increment RREQ hop count
  // uint8_t hop = rreqHeader.GetHopCount () + 1;
  // rreqHeader.SetHopCount (hop);
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  int64_t temp = 0;
  temp = rreqHeader.GetRTTstart();
  rrepHeader.SetRTTstartRrep(temp);
  ...
  TypeHeader tHeader (wprTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), wpr_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (wprTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), wpr_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  sumPPTjni = 0;
  countForAveragePPT = 0;
  sumTimeDifference = 0;


  NS_LOG_FUNCTION (this << " to " << neighbor);
  RrepAckHeader h;
  TypeHeader typeHeader (wprTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, wpr_PORT));
}
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  newEntry.SetRTTstartRrep(rrepHeader.GetRTTstartRrep());
  listRTT.push_front(newEntry);


  ofstream stream;
  string const HOME = std::getenv("HOME") ? std::getenv("HOME") : ".";
  string path = "/repos/ns-3-allinone/ns-3-dev/outputTextFiles/rtt/";
  stringstream ss;
  stringstream ss2;

  ss << newEntry.GetInterface().GetLocal();
  stream.open(HOME + path + ss.str().c_str() + ".txt", fstream::app);
  ss2 << Simulator::Now().GetMilliSeconds() - rrepHeader.GetRTTstartRrep();
  stream << ss2.str().c_str();
  stream << '/';
  ss2.clear();
  ss2 << rrepHeader.GetDstSeqno();
  stream << ss2.str().c_str();
  stream << endl;
  stream.close();

  //cout << Simulator::Now().GetMilliSeconds() - rrepHeader.GetRTTstartRrep() << endl;
  

  //flag rtt attempts?
  
  //ifstream ifs;
  //ifs.open(HOME + path + ss.str().c_str() + ".txt");
  
  //int a, b, rttvalue;
  //while(ifs >> a >> b >> rttvalue){
    //cout << "infile a: " << a << " and b: " << b << endl;
  //}
  //cout << " Size of the rtt list is: " << listRTT.size() << endl;
  
  //can just sort list by making two lists then popping iteratively and pushing front or back based on whether bigger than peek front
  /*
  int tempcompare = -1, listrttsize = listRTT.size();
  list<RoutingTableEntry> sortedList;

  for(int i = 0; i < listrttsize; i++){
    if(tempcompare = -1){
      tempcompare = listRTT.front().GetRTTstartRrep();
      sortedList.push_front(listRTT.front());
    } else {
      if(sortedList.front().GetRTTstartRrep >= listRTT.front().GetRTTstartRrep()){
        sortedList.push_front(listRTT.front())
      } else {
        sortedList.push_back(listRTT.front());
      }
      listRTT.pop_front();
    }
  }
  */
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    ...
    {/*
      list<RoutingTableEntry> tempiterator = listRTT;


    //tempswap.SetDstSeqno(tempswap.GetDstSeqno()+10);
    std::list<RoutingTableEntry>::iterator it;
  //og
    for(auto const& i : listRTT){
      if(dst.Get() == i.GetDestination().Get()){
        //cout << "STAAAAAAAAAAAAAAAAAAAAAAAAAAAAAHP" << endl << endl;
        //exit(1);
        //Simulator().Stop();
        SendReply(tempswap, i);
        break;
    }
  ...
  if (m_routingTable.LookupRoute (neighbor, rt))
    {
      //flag rtt
      ofstream streamd;
      string const HOMEd = std::getenv("HOME") ? std::getenv("HOME") : ".";
      string pathd = "/repos/ns-3-allinone/ns-3-dev/outputTextFiles/rtt/10.1.1.";
      stringstream ss;
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (wprTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
...
/*
  list<RoutingTableEntry> tempiterator = listRTT;
//sigiot dont work, retry rrep packet with dest/src confirmation first
  ...
  //the RTT array has list of different dst/sourcees per entry, check if insearch and dst before saving in RTT list
/*
  TypeHeader tHeader (wprTYPE_RREQ);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
...
*/
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (wprTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, wpr_PORT));
        }
    }


//flag list rtt array STAAAAAAAAAAAAAAAAAA
    SendHello();
