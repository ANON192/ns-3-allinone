 ...
 *
 * Based on
 *      NS-2 clafaodv model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      clafaodv-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/clafaodv-UU
 *
...
#include "clafaodv-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("clafaodvRoutingProtocol");

namespace clafaodv {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for clafaodv control traffic
const uint32_t RoutingProtocol::clafaodv_PORT = 654;

/**
* \ingroup clafaodv
* \brief Tag used by clafaodv implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::clafaodv::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("clafaodv")
...
{
  static TypeId tid = TypeId ("ns3::clafaodv::RoutingProtocol")
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        ...
                        << ", clafaodv Routing table" << std::endl;
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No clafaodv interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // clafaodv is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == clafaodv_PORT)
                    {
                      // clafaodv packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("clafaodv does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvclafaodv, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), clafaodv_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::Recvclafaodv, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), clafaodv_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("clafaodv does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in clafaodv operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when clafaodv does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // clafaodv needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available clafaodv interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid clafaodv source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Send RREQ as subnet directed broadcast from each interface used by clafaodv
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (clafaodvTYPE_RREQ);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, clafaodv_PORT));
...
void
RoutingProtocol::Recvclafaodv (Ptr<Socket> socket)
{
...
//Find where neighbor is invoked and just pass the new neighbor list back and forth from there to make it work
  ...
  NS_LOG_DEBUG ("clafaodv node " << this << " received a clafaodv packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("clafaodv message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
    case clafaodvTYPE_RREQ:
      {
        RecvRequest (packet, receiver, sender);
        break;
      }
    case clafaodvTYPE_RREP:
      {
        RecvReply (packet, receiver, sender);
        break;
      }
    case clafaodvTYPE_RERR:
      {
        RecvError (packet, sender);
        break;
      }
    case clafaodvTYPE_RREP_ACK:
      {
        RecvReplyAck (sender);
        break;
      }
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (clafaodvTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), clafaodv_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (clafaodvTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), clafaodv_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (clafaodvTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, clafaodv_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (clafaodvTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (clafaodvTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, clafaodv_PORT));
        }
    }
}
